...
void savePunchToPC(const Punch& punch);
void clearFolder(const std::string& folderPath);
void savePunchToFavoritesDirectory(const Punch& punch);
                    ...
                    if (ImGui::BeginPopup(popup_id.c_str()))                                        // Ensurring the popup ID matches.
                    {
                        ...
                        if (!alreadyInFavorites)
                        {
                            if (ImGui::MenuItem("Save To Favorites"))
                            {
                                ...
                                savePunchToFavoritesDirectory(rec);
                            }
                        }
                        else
                        {
...
void savePunchToPC(const Punch& punch) {        // Function to save punch to PC.
    std::string filePath = "punchesFile/punch_" + std::to_string(punch.id) + ".txt";
...
void clearFolder(const std::string& folderPath) {   // Function to ctear a folder from its files.       
...
void savePunchToFavoritesDirectory(const Punch& punch) {    // Function to save a punch to the favorites directory, as a file.
    std::string filePath = "favorites/punch_" + std::to_string(punch.id) + ".txt";
    std::ofstream outFile(filePath);
...
void initializeFavorites(CommonObjects* common) {       // Function to be called in the start of the program.
    common->favorites.clear();                          // Clearing the favorites vector, because in the start of the program it reloads the favorites from 
